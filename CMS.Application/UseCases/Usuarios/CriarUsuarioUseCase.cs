...
namespace CMS.Application.UseCases.Usuarios
{
    public class CriarUsuarioUseCase
    {
        ...
        public CriarUsuarioUseCase(IUsuarioRepository usuarioRepository, IPermissaoUsuario permissaoUsuario)
        {
        ...
        public async Task<ResponseDto<string>> ExecuteAsync(string nome, string email, string senhaPura, string papel)
        {
            ...
            if (!_permissaoUsuario.PodeCriarUsuario())  
            {
                return ResponseDto<string>.Falha("Você não tem permissão para criar usuários.");
            }
